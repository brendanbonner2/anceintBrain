...
function    aStar(){
    ...
    var openSet = new Array(GRID_SIZE);
    var closedSet = new Array(GRID_SIZE);
    
    openSet.push(gridSpot[ei][ej]); // push startnode onto Openset
    gridSpot[ei][ej].f = 0;
    ...
    while(openSet.length > 0)
    {
        ...
        {
            for (child = currentNode.length; child >0; child++)
            {
...
{
    if (diagonal) 
        return ( Math.pow(Math.abs(ai - ei),2) + Math.pow(Math.abs(aj - ej),2));
    else
        return ( Math.abs(ai - ei) + Math.abs(aj - ej));
}
...
function    consoleGrid(){
    ...
    for (var i = gridsize-1; i>= 0 ; i--){
        ...
        {
            switch (GRID[i][j]){
                case GRID_WALL: gridString = gridString + "L"; break;
                case GRID_MAZE: gridString = gridString + "X"; break;
                case GRID_AGENT: gridString = gridString + "A"; break;
                default:
        ...
        console.log("Heuristic: " + aStarHeuristics());
}
